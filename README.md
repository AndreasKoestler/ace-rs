...
- **OQ-1 (toolchain):** `is_x86_feature_detected!("avxvnniint16")` and all six `_mm256_dpw*_epi32` word intrinsics (plus the five new byte intrinsics) resolve on **stable Rust 1.96** — no MSRV bump and no nightly feature flags. Re-confirmed with `cargo check --target x86_64-unknown-linux-gnu`.
